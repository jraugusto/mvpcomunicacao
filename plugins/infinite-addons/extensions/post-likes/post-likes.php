...
// Template Tags -------------------------------------------------------
function rella_likes_button( $args = array(), $post_id = 0 ) {

	// if post support post likes
	if( !post_type_supports( get_post_type( $post_id ), 'rella-post-likes' ) ) {
		echo __( 'Post type not support likes.', 'infinite-addons' );
		return;
	}
...
class Rella_Post_Like extends Rella_Base {
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'rella-likes', RA_ADDONS_URL . '/extensions/post-likes/rella-likes.js', array( 'jquery' ), '0.1', true );
	}
	...
	public function save_like() {
		...
		if ( ! wp_verify_nonce( $nonce, $this->noncekey ) ) {
			wp_send_json_error( esc_html__( 'Not permitted', 'infinite-addons' ) );
		}

		// Get post_id
		$post_id = isset( $_REQUEST['post_id'] ) && is_numeric( $_REQUEST['post_id'] ) ? $_REQUEST['post_id'] : '';
		if( ! $post_id ) {
			wp_send_json_error( esc_html__( 'No Post ID', 'infinite-addons' ) );
		}

		// if post support post likes
		if( !post_type_supports( get_post_type( $post_id ), 'rella-post-likes' ) ) {
			wp_send_json_error( esc_html__( 'Post type not support likes.', 'infinite-addons' ) );
		}
	...
	public function get_likes_button( $args = array(), $post_id = 0 ) {
		...
		$defaults = array(
			...
			'format' => wp_kses_post( __( '<span class="post-likes-count">%s</span> Likes', 'infinite-addons' ) ),
		);
