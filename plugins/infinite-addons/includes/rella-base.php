...
class Rella_Base {
	...
	public function load_extension( $name = '' ) {
		...
		if( !$located ) {
			wp_die( sprintf( __( '<strong>Extension Load Fail for:</strong> %1$s<br>No such file "%2$s" exists.', 'infinite-addons' ), strtoupper( $name ), "rella/extensions/{$name}/rella-{$name}.php" ), esc_html__( 'Load Extension', 'infinite-addons' ), null );
			return;
		}
