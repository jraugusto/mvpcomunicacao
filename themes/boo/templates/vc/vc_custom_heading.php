 ...
 */
$source = $text = $link = $google_fonts = $font_container = $el_class = $css = $css_animation = $font_container_data = $google_fonts_data = $letter_spacing = $gradient_color = $enable_fittext = $fittex_size = '';
...
if ( apply_filters( 'vc_custom_heading_template_use_wrapper', false ) ) {
	$output .= '<div class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $data_opts ) . ' >';
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' >';
	$output .= wpb_js_remove_wpautop( do_shortcode( $text ), false );
	$output .= '</' . $font_container_data['values']['tag'] . '>';
	$output .= '</div>';
} else {
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $data_opts ) . ' >';
	$output .= wpb_js_remove_wpautop( do_shortcode( $text ), false );
	$output .= '</' . $font_container_data['values']['tag'] . '>';
}
